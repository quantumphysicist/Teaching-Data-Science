...
SORT_BY_LAST_NAME = False  # Set to True to sort by last name. Set to False to sort by first name.
...
if SORT_BY_LAST_NAME:
    # Split names, sort by last name, and recombine first and last names
    attendance_df['First Name'] = attendance_df['Name'].str.split().str[0]
    attendance_df['Last Name'] = attendance_df['Name'].str.split().str[-1]
    attendance_df = attendance_df.sort_values('Last Name').drop(columns=['Name'])
    attendance_df['Name'] = attendance_df['First Name'] + ' ' + attendance_df['Last Name']
    attendance_df = attendance_df.drop(columns=['First Name', 'Last Name'])
